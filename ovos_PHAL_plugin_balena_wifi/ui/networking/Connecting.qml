...
Item {
    ...
    onConnectedStatusChanged: {
        if(connectedStatus == 1) {
            ...
            console.log("Sending success to phal plugin")
            Mycroft.MycroftController.sendRequest("ovos.phal.balena.ondevice.report.success", {})
        }
    }
    ...
    onDisconnectedStatusChanged: {
        if(disconnectedStatus == 1 && !connectingView.connectionError) {
            networkingLoader.clear()
            console.log("Sending fail to phal plugin")
            Mycroft.MycroftController.sendRequest("ovos.phal.balena.ondevice.report.fail", {})
